...
<title>Απεγκατάσταση Πίνακα</title>
<meta charset="UTF-8">
...
<div class="des" style="position: relative;margin-right: 10px; margin-top: 50px;padding:8px 2px 12px 9px;width:50%;">
<?php
include('auth.inc');
...
if (!@mysqli_query($link,$query)) {
        echo "<h2><center> Έχει γίνει ήδη απεγκατάσταση!</center></h2> <br>";
 }
else{
       $query="drop table if exists meds;";
       if (@mysqli_query($link,$query)) {
        echo "<h2><center> O πίνακας απεγκαταστάθηκε επιτυχώς!</center></h2> <br>";
 } else {
        echo "<h2><center>Ο πίνακας δεν απεγκαταστάθηκε:</center></h2>".mysqli_error($link)."<br>";
}
}
 mysqli_close($link);
?>
