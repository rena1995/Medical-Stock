...
?>
<h2><center>Αναζήτηση με βάση τo barcode</h2></center>
<FORM ACTION="<?=$_SERVER['PHP_SELF']?>" METHOD=POST >
<br>
<center><INPUT TYPE="text" NAME="search" value="" placeholder="0000000000000"  pattern="[0-9]{13}" size="15" required></center>
<br>
...
$srch=$_GET['search'];
$query = "select * from meds where barcode like '".$srch."%'";
$result=NULL;
...
$count=mysqli_num_rows($result);
 ...
 echo "<li> <a href=\"search_bar.php?search=$srch&offset=0\">Πρώτο</a></li>";
 for ($i=0;$i<$page_number;$i++) {
        $mf=$i*$limit;
        $num=$i+1;
        if ($off>=$i*$limit && $off<($i+1)*$limit)
        echo "<li><a href=\"search_bar.php?search=$srch&offset=$mf\"
 class=\"active\">[$num]</a></li>";
        else
        echo "<li><a href=\"search_bar.php?search=$srch&offset=$mf\">[$num]</a></li>";
        }
 echo "<li> <a href=\"search_bar.php?search=$srch&offset=$mf\">Τελευταίο </a></li>";echo "</ul>";
 $query = "select * from meds where barcode like '".$srch."%' limit $off, $limit";
 $result = mysqli_query($link,$query) or die(mysqli_error());
...
echo "</tbody></table>";
 ...
 echo "<P><center><br><h4>Δεν υπάρχει φάρμακο με αυτό το barcode</h4></br></center></P>";
 }
 mysqli_close($link);
}
